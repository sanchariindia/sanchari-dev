...
if(isset($id) && $id!=''){
 ...
 $row = $db->getOne($tblBanner);	
		...
		if($file_name!=''){
			...
			move_uploaded_file($_FILES["icon"]["tmp_name"],"../../assets/banner/".$img);
			//$imagepath = $img;
	...
	$db->where("id",$id);
	$insertId = $db->update($tblSlider,array("image"=>$img,"status"=>1));

	if($insertId){
		$_SESSION["msg"] ="Successfully Update";
		header('location:../addappslider.php');
	}else{
		...
		if($file_name!=''){
			...
			$insertId = $db->insert($tblBanner,array("image"=>$img,"status"=>1));
		}
	if($insertId){
